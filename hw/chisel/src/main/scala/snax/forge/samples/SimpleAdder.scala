...
/** Streaming interface for a two-input, one-output elementwise operator.
  ...
  * any other Data subclass. Modules are unaffected -- Module is not Data -- which is why SimpleAdder below can still
  * call its parameter `width`.
  */
...
/** Lane-parallel integer adder -- the hardware image of `kernels/polybench/vecadd.py`.
  *
  * Purely combinational: `out.bits` is a function of the current inputs, and the only state is the absence of state.
  * This is the degenerate case of the latency-balancing problem the thesis is actually about (L = 0, II = 1), and it is
  * deliberately the first thing to work end to end.
  *
  * Handshake: a join. `out` fires only when both inputs are valid, and each input is consumed only when the other input
  * and the consumer are also ready. Note that `a.ready` depends on `b.valid` and vice versa -- that is legal (ready may
  * depend on other channels' valid) whereas making `valid` depend on its own `ready` would deadlock.
  *
