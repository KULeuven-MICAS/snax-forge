...
/** Elaboration entry point: Chisel -> CHIRRTL -> firtool -> SystemVerilog.
  ...
  *
  * This object is hand-written today and generated tomorrow. Once the Accelerator Descriptor schema freezes in W5, the
  * emitter will read a descriptor and choose the module and its parameters from it; the shape of this file is what that
...
object Emit {
  ...
  /** `gen` is by-name: Chisel requires the module to be constructed *inside* the elaboration context, so passing an
    * already-built instance throws.
    */
