...
with skipping on and off. Scenarios are checked in under `scenarios/` and
generated by `scenarios/make.py`, which is the only place block-level helpers
expand into register writes.
...
5. Streamer dynamic TCDM priority and reader repeat on temporal stride 0 (D32): copy or keep out, decided with the deferred anchor (D51); not copied until then.
...
24. `scenarios/make.py` holds every scenario's layout and program by hand, plus shared helpers (`Program`, `contiguous`, controller costs, cluster builders), and grows with each scenario. Plan: split it into a package `scenarios/make/` (shared module, one module per cluster and per scenario, a registry so `make.py` and `--check` keep working) when it gets in the way; in M3, move the shared parts into SNAX-LOWER (LOW1b: task list → commands, LOW1c: cluster files, D45, D53) and write scenarios as hand-written task lists once open item 19 fixes their format. LOW1b did its part (D64): `Program` lives in `snax_forge/lower/`, and every scenario but fmul is written as a task list. Left: the cluster builders (`alu4`, `red4`, `mul1`, `_streamer`, `CTL`) move with LOW1c, the layout helpers (`contiguous`, `unit`, fmul's) with BRM2 / LOW1a.
...
26. `scenarios/fmul` is scheduled by hand with `Program`, not written as a task list (D64): its program waits twice on a DMA that was already waited for (before loading a of tile 1 and before storing c of the last tile) and configures the last store after its waits. Lowered from a task list it runs 503 cycles instead of 525, with the same output. Switch it when changing its numbers, and VIS3's check at cycle 65, is acceptable.
