...
with skipping on and off. Scenarios are checked in under `scenarios/`, one
folder each with the `scenario.py` that makes it; `scenarios/make.py` writes
their files (D65). Most programs are lowered from a hand-written task list in
the same folder (D64); block-level helpers expand into register writes only
there, through SNAX-LOWER, never inside the model.
...
26. `scenarios/fmul` is scheduled by hand with `Program` in `fmul/scenario.py`, not written as a task list (D64, D65): its program waits twice on a DMA that was already waited for (before loading a of tile 1 and before storing c of the last tile) and configures the last store after its waits. Lowered from a task list it runs 503 cycles instead of 525, with the same output. Switch it when changing its numbers, and VIS3's check at cycle 65, is acceptable.
