...
- SNAX-LOWER (M3, D64) lives in snax_forge/lower/ (task list, per-type values, lowering to commands, the `Program` command builder), tests in tests/lower/ (a package, like tests/viz). Every scenario but fmul is written in scenarios/make.py as a task list and checked in as `tasks.json` beside its `scenario.json`.
...
gives exactly the program of `scenarios/vecadd/scenario.json`. fmul stays
scheduled by hand (open item 26).
