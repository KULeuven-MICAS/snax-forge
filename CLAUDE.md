...
- Tests: `pixi run test` (everything under tests/), `pixi run test-model` (SNAX-MODEL only), `pixi run test-lower` (SNAX-LOWER only); CI runs `pixi run -e ci test`
...
- One package per component under `snax_forge/`: snax_model and viz (built), lower (task list → program built, D64), then dfg, brm, dse.
- Scenarios: one folder per scenario under `scenarios/`, with the `scenario.py` that makes it and, for most, a hand-written `tasks.json` it lowers into the program (D64, D65). `pixi run scenarios` (`python scenarios/make.py`, `--check` to compare) writes `scenario.json`, the `.npy` data and the cluster files; never edit those by hand.
